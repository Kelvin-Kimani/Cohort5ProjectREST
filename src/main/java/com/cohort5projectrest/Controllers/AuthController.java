        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")){
            ...
            try {
                ...
                String access_token = JWT.create()
                        .withSubject(user.getEmployeeEmailAddress())
                        .withExpiresAt(new Date(System.currentTimeMillis() + 10 * 60 * 1000))
                        .withIssuer(request.getRequestURL().toString())
                        .withClaim("roles", Arrays.stream(user.getUserRole().split(",")).collect(Collectors.toList()))
//                        .withClaim(claims)
                        .sign(algorithm);
                ...
                log.info("Successfully given new access token");
